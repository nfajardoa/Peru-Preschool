...
dataset_ses <- tbl(db_conn, 'regression_table') %>% 
  ...
  arrange(numero, periodo, desc(first_born), pyear) %>%
  mutate(
    older_itt = if_else(row_number() == 1 & preschool_itt == 0, TRUE, FALSE),
    older_ate = if_else(row_number() == 1 & preschool_ate == 0, TRUE, FALSE),
    constant_itt = base::xor(preschool_itt, dplyr::lag(preschool_itt, 1)),
    constant_itt = sum(constant_itt, na.rm = TRUE) %>% `<=`(1),
    constant_ate = base::xor(preschool_ate, dplyr::lag(preschool_ate, 1)),
    constant_ate = sum(constant_ate, na.rm = TRUE) %>% `<=`(1)
  )%>%
  fill(older_itt, older_ate, .direction = "down") %>%
  ungroup() %>%
  mutate(
    switcher_itt.corr = dplyr::select(., switcher_itt, constant_itt, older_itt) %>% rowSums() %>% `==`(3),
    switcher_ate.corr = dplyr::select(., switcher_ate, constant_ate, older_ate) %>% rowSums() %>% `==`(3),
